...
Think back to the simulations we looked at to understand the Central Limit Theorem (CLT). Recall that populations were always more variable than sample means (except in the special case where n=1). After all, sample means are "middles", and the middles will cluster together more than individuals. It is more likely for an individual person to have a longer-than-average thumb, than for a random sample of 157 people to all have longer-than-average thumbs? 
