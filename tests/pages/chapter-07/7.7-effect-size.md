...
So far we have been talking a lot about a model using **Sex** as the explanatory variable. The **Sex** model is an example of what we could call a "two-group model" because there are two values of sex in this data set and thus two groups of data. We’ve been thinking about trying to measure how good (or bad) our model is, but there is another way to think about this. 
...
Similar to the ```b1()``` function, the ```PRE()``` function will return the PRE for a model. In the DataCamp window below run ```supernova(Thumb~Sex, data=Fingers)```. Look in the supernova table to see what the PRE is for the **Sex** model. 
